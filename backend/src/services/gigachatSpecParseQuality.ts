 ...
 * Ключевой различитель коррупции от валидной спеки — именно ПЕРВЫЙ признак
 * (почти все quantity=null). Настоящий номер позиции тоже целое 1..N, т.е.
 * количество-образен, поэтому posAsQty сам по себе валидную спеку не отличает;
 * у здоровой спеки количества заполнены (nullQtyFraction низкая) и блок не
 * срабатывает, даже если у части родителей количество законно пустое.
 */
