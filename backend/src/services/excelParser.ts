...
/**
 * Подпись строки заголовков — по ней узнаём «тот же самый бланк» и переиспользуем
 * разметку колонок, размеченную человеком руками.
 *
 * Нормализация: trim, схлопывание пробелов, нижний регистр, отбрасывание пустых
 * ХВОСТОВЫХ ячеек (Excel дописывает их сколько хочет). Сравнивать подписи можно
 * ТОЛЬКО целиком: частичное совпадение шапки — это чужая разметка, подставленная
 * молча, что хуже, чем её отсутствие.
 */
export function headerSignature(cells: unknown[]): string {
  const norm = (cells ?? []).map(c => String(c ?? '').replace(/\s+/g, ' ').trim().toLowerCase());
  while (norm.length > 0 && norm[norm.length - 1] === '') norm.pop();
  return norm.join('\u0001');
}
