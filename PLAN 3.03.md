...
| 2 | И-1/2 | ✅ Выполнен | `см. ниже` |
| 3 | И-7 | ⬜ Не начат | — |
| 4 | И-8 | ⬜ Не начат | — |
| 5 | И-5, И-11 backend | ⬜ Не начат | — |
...
### ⬜ Этап 2 — И-1/2: ColumnMapper UX (~4 ч)
...
### ⬜ Этап 3 — И-7: Фиксы парсера (~3.5 ч)

**Задачи:**
1. `pdfParser.ts` / `excelInvoiceParser.ts`: убрать `'доставка'`, `'транспортн'` из `SKIP_ROW_KEYWORDS` (готовим к этапу 4)
2. `detectColumns()`: повысить порог — требовать `name` + **2** дополнительных поля (вместо 1)
3. Добавить негативные паттерны: строки с «БИК», «ИНН», «адрес» не считаются заголовками таблицы
4. При категории B/C: улучшить `parsing_category_reason` — записывать конкретную причину

**Файлы:**
- `backend/src/services/pdfParser.ts`
- `backend/src/services/excelInvoiceParser.ts`

**Commit:** `fix: parser strictness — column detection threshold +2, remove delivery from skip, better category reason`
...
### ⬜ Этап 9 — И-4 frontend: UI проверки единиц (~4 ч)
...
### ⬜ Этап 11 — И-9 frontend + матчинг (~4 ч)
...
### ⬜ Этап 13 — И-16: Иерархические позиции DN (~4 ч)
