...
INCH_DN = {"3/8": 10, "1/2": 15, "3/4": 20, "1": 25, "1 1/4": 32, "5/4": 32, "1 1/2": 40, "2": 50,
           "2 1/2": 65, "3": 80, "4": 100}
...
# DN/Ду/Dy (латиница, кириллица и их смесь: Dу, DУ) с числом после любого не-буквенного
# символа (VFM-2R/Dy32/Kvs16); «DN 1/2», «DN 1"1/2» и «ДУ 2“» — это дюймы, их берёт INCH.
# Переходник/диапазон: второй типоразмер через x/х/×/-/пробел — Ду15-50, Ду25х20
DN = re.compile(r'(?<![A-Za-zА-Яа-яЁё])(?:d[nyу]|д[уy])\s*[-.]?\s*(\d{1,4})'
                 r'(?:[xх×\-\s]+(\d{1,4}))?(?![\d"]|/\d|\s*"|\s+\d/\d)', re.I)


def dn_sizes(text):
    """Типоразмеры DN из текста: DN/Ду/Dy с числом и дюймы, приведённые к DN.

    Переходник/диапазон отдаёт ДВА размера (1/2"x3/4", Ду15-50, Ду25х20) — не один.
    """
    s = html.unescape(html.unescape(text or ""))          # у продавцов бывает 1/2&amp;quot;
    s = re.sub(r"''|[″“”'`]", '"', s)
    # разделитель размеров сразу после дюймовой кавычки (1/2"x3/4", 1/2"-2") -> пробел,
    # чтобы обе половины разобрал один и тот же проход INCH ниже
    s = re.sub(r'"\s*[xх×\-]\s*(?=\d)', '" ', s)
    # 1"1/2, 1" 1/2 -> 1 1/2" (лукбехайнд не даёт зацепить хвост дроби из «1/2" 3/4"»:
    # там перед "2" и "4" стоит "/", склейка не срабатывает — это два разных размера)
    s = re.sub(r'(?<![/\d])(\d+)"\s*(\d/\d)(?!\d*")', r'\1 \2"', s)
    out = set()
    for n1, n2 in DN.findall(s):
        out.add(int(n1))
        if n2: out.add(int(n2))
...
def size_ok(spec_text, card_text):
    ...
    Позиция сама переходник (два размера, «Ду25х20») — совпадение только при полном
    равенстве множеств размеров позиции и карточки, иначе None (не гадаем какой из двух).
...
def _selfcheck():
    ...
    # Ф9.5а (1): переходник/диапазон даёт ДВА размера, не один
    assert dn_sizes('1/2"x3/4"') == {15, 20}
    assert dn_sizes('1/2" 3/4"') == {15, 20}
    assert dn_sizes('1/2"-2"') == {15, 50}
    assert dn_sizes("Ду15-50") == {15, 50}
    assert dn_sizes("Ду25х20") == {25, 20}
    assert size_ok("Переходник Ду25х20", "Переходник Ду25х20 латунный") is True
    assert size_ok("Переходник Ду25х20", "Переходник Ду32х25") is None
    # Ф9.5а (2): «Dу»/«DУ» смешанным алфавитом (латиница D + кириллица у/У)
    assert dn_sizes('Dу65/Py16') == {65}
    assert dn_sizes("DУ65") == {65}
    # Ф9.5а (3): дюймы 3/8" -> Ду10, 5/4" -> Ду32
    assert dn_sizes('Кран 3/8"') == {10} and dn_sizes('Кран 5/4"') == {32}
    assert dn_sizes('3/8"') == {10}
    # Ф9.5а раунд 2: регрессия d9ded03 — склейка «1" 1/2» без пробела давала {25} вместо {40}
    assert dn_sizes('1" 1/2') == {40}
    assert dn_sizes('1"1/2') == {40}
    assert dn_sizes('1 1/2"') == {40}
    assert dn_sizes('Кран DN 1"1/2') == {40}
    assert dn_sizes('DN 1"1/4') == {32}
    assert dn_sizes('1/2" 3/4"') == {15, 20}
    assert dn_sizes('1/2"x3/4"') == {15, 20}
    assert dn_sizes("Ду15-50") == {15, 50}
    assert dn_sizes('Dу65/Py16') == {65}
    assert dn_sizes('MNF-R2 PN25') == set()
    assert dn_sizes('ISO 7/1') == set()
    assert dn_sizes('VFM-2R/Dy32/Kvs16') == {32}
    # защита от поломки — прежние отказы остаются отказами
    assert dn_sizes("MNF-R2 PN25") == set() and dn_sizes("ISO 7/1") == set()
    assert dn_sizes("VFM-2R/Dy32/Kvs16") == {32}
